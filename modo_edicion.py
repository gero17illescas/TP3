...
class Shell(cmd.Cmd):
	...
	def do_trackadd (self, parametros):
		...
		volumen = validar_entero(volumen)
	...
	def do_markadd(self,duration):
		...
		editor.timeline.append({"duracion":int(duration)/100})
		editor.avanzar(1)
	...
	def do_markaddnext(self, duration):
		...
		duration = validar_entero(duration)
		editor.timeline.insert(editor.index+1,{"duracion":int(duration)})
	...
	def do_markaddprev(self,duration):
		...
		print(editor.index)
		editor.timeline.insert(editor.index-1,{"duracion":int(duration)/100})
